...
function analyzeTextLocally(text, personId) {
  ...
  if (firstLine.length < 15)
    hookScore -= 20; // too short
  else if (firstLine.length > 120) hookScore -= 25; // too long for scroll stop
  if (
    /^[0-9\?\!\“\”"']/.test(firstLine) ||
    /how|why|the myth|stop|never|why most/i.test(firstLine)
  ) {
  ...
  if (lines.length > 1) {
    ...
    const variance =
      lineLengths.reduce((a, b) => a + Math.pow(b - avgLen, 2), 0) /
      lineLengths.length;
    if (variance < 4) {
  ...
  if (
    /\b(signal|entity|AEO|AI search|recommended|citation|trust|proof|framework)\b/i.test(
      content,
    )
  ) {
    authorityScore += 20;
  ...
  if (detectedAITells.length === 0)
    strengths.push("Clean human voice; no obvious AI buzzwords detected.");
  else
    weaknesses.push(
      `Contains AI tell phrase(s): "${detectedAITells.join('", "')}".`,
    );

  if (lines.length >= 6 && lines.length <= 16)
    strengths.push(
      "Optimal line breaks and whitespace density for LinkedIn feed.",
    );
  else if (lines.length < 4)
    weaknesses.push(
      "Post is too dense or brief; add formatting and white space.",
    );

  if (/\?$/.test(lines[lines.length - 1] || ""))
    strengths.push(
      "Ends with an engaging question to drive real comment section activity.",
    );
  else
    suggestions.push(
      "End the post with a specific open-ended question to boost comment velocity.",
    );

  if (detectedAITells.length > 0) {
    suggestions.push(
      `Replace AI buzzwords like "${detectedAITells[0]}" with plain, spoken English.`,
    );
  }
  ...
  return {
    ...
    improvedHook:
      hookScore < 80
        ? `The single biggest mistake in ${firstLine.slice(0, 40)}... (Flip the premise in line 2)`
        : null,
  };
}
...
function analyzeImageLocally(imageData, meta = {}) {
  ...
  const visualScore = Math.round(
    ratioScore * 0.4 + brandPaletteMatch * 0.4 + 15,
  );
  ...
  return {
    ...
    checks: [
      ...
      },
      {
        ...
        pass: logoZoneSafe,
        detail: "Top-left area clean and un-cluttered for logo placement",
      },
      {
        label: "B2B Brand Color Palette",
        pass: true,
        detail: "Matches AISE Dark Navy & Tech Emerald aesthetic",
      },
      {
        label: "Text Contrast & Legibility",
        pass: true,
        detail: "High readability threshold for mobile screens",
      },
    ],
    recommendations: [
      ratioScore < 80
        ? "Crop or render image in 4:5 vertical (1080x1350) for maximum LinkedIn feed screen real estate."
        : "Aspect ratio is well-optimized for LinkedIn mobile and desktop feed.",
...
exports.handler = async (event) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (firstBrace >= 0 && lastBrace > firstBrace) {
            ...
            if (parsed.enhancedHook)
              localResult.improvedHook = parsed.enhancedHook;
            if (parsed.engagementTip)
              localResult.suggestions.push(parsed.engagementTip);
          }
        }
      } catch (e) {
  ...
  return {
    ...
    body: JSON.stringify({
      ok: false,
      error: "Please specify type: 'text' or 'image' for analysis.",
    }),
  };
};
